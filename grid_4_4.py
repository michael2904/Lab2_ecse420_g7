...
for itera in range(T):
	...
	if rank == 0:
		...
		for i in range(1,N-1):
			...
			uM[0][i] = results1[count3Mod+0][count3Val]
			uM[N - 1][i] = results1[count3Mod+1][count3Val]
			uM[i][0] = results1[count3Mod+2][count3Val]
			uM[i][N - 1] = results1[count3Mod+3][count3Val]
			count3 += 1
