...
for itera in range(T):
	...
	if rank == 0:
		...
		for i in range(1,N-1):
			dataN1[(((i-1) + 0*4)/size) % size].append(uM[1][i])
			dataN1[(((i-1) + 1*4)/size) % size].append(uM[N - 2][i])
			dataN1[(((i-1) + 2*4)/size) % size].append(uM[i][1])
			dataN1[(((i-1) + 3*4)/size) % size].append(uM[i][N-2])
			print("*********This is rank "+str(rank)+" and dataN1 "+str(dataN1))
	else:
		dataN1 = None
	...
	if rank == 0:
		...
		for i in range(1,N-1):
			...
			uM[0][i] = results1[count3Mod][count3Val]
			uM[N - 1][i] = results1[count3Mod][count3Val]
			uM[i][0] = results1[count3Mod][count3Val]
			uM[i][N - 1] = results1[count3Mod][count3Val]
			count3 += 1
